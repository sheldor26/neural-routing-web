...
export default function FullChatPage() {
  ...
  const renameSession = async (sId: string, title: string) => {
    ...
    try {
      await fetch(`https://web-production-4f439.app/v1/sessions/${sId}/rename`, {
        method: 'PATCH',
  ...
  const handleSendMessage = async (overridePrompt?: string) => {
    ...
    try {
      ...
      if (response.status === 402) {
        setMessages(prev => [...prev, { role: 'assistant', content: "⚠️ **Limit Reached.** Staying on Free is costing you money. Upgrade to resume routing." }]);
        return;
      }

      if (data.output?.ai_answer) {
        const cost = data.business_metrics?.cost_usd || 0;
        const gpt4Cost = data.business_metrics?.estimated_gpt4_cost || 0.001;
        ...
        setMessages(prev => [...prev, {
          ...
          stats: {
            ...
            reasoning: isOverdrive ? "High complexity detected. Neural Overdrive engaged for precision." : (data.routing_decision?.reason || "Optimized cost route.")
          }
        }]);

        // Auto-rename session if it's the first message
        if (messages.length === 0) {
          renameSession(currentSessionId, p);
        } else {
          fetchSessions();
        }
      }
  ...
  const shareSavings = (pct: number, overdrive: boolean) => {
    const text = overdrive ? `Maximum precision achieved via Neural Overdrive ⚡` : `I just saved ${pct.toFixed(0)}% using NeuralRouting.io 🚀`;
    navigator.clipboard.writeText(text);
    alert("Copied to clipboard!");
  };

  return (
    <div className="flex h-screen bg-[#050506] text-zinc-300 overflow-hidden relative text-white">
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-2 pb-10">
          ...
          {sessions.map((sess) => (
            ...
            >
              ...
              <div className="text-[8px] text-emerald-500 mt-1 font-bold">VALUE GENERATED</div>
            </div>
          ))}
        </nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative w-full overflow-hidden">
        <header className="h-24 border-b border-white/5 flex items-center justify-between px-4 md:px-8 bg-[#050506]/80 backdrop-blur-xl z-10">
           <div className="flex items-center gap-6">
             ...
             <div className="flex flex-col gap-2">
               ...
               <div className="w-48 h-1 bg-white/5 rounded-full overflow-hidden border border-white/5">
                  <div className="h-full bg-blue-600 transition-all duration-1000" style={{ width: `${Math.min((stats.weekly / WEEKLY_GOAL) * 100, 100)}%` }} />
               </div>
             </div>
           </div>
           <div className="flex items-center gap-4">
             <Link href="/dashboard" className="hidden sm:flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white bg-blue-600 px-6 py-3 rounded-full hover:scale-105 transition-all shadow-lg shadow-blue-600/20">
                Integrate API <ArrowUpRight size={14} />
             </Link>
          ...
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-8 animate-in fade-in zoom-in-95">
               ...
               <div>
                 ...
                 <p className="text-zinc-500 text-sm font-medium uppercase tracking-widest">Saving up to 90% per request automatically.</p>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
                  {["Draft professional email", "Audit technical code"].map((t, idx) => (
                    ...
                    <div className="mt-8 pt-8 border-t border-white/5 space-y-6">
                      <div className="flex flex-col gap-1">
                        <h4 className={`text-[10px] font-black uppercase tracking-widest italic flex items-center gap-2 ${m.stats.isOverdrive ? 'text-blue-500' : 'text-emerald-500'}`}>
                           ...
                           {m.stats.isOverdrive ? "Neural Overdrive: Max Precision" : `Paid ${m.stats.savings_pct.toFixed(0)}% less than GPT-4`}
                        </h4>
                        <p className="text-[9px] text-zinc-500 font-bold uppercase">Cost: ${m.stats.cost.toFixed(5)} vs ${m.stats.gpt4_cost.toFixed(5)}</p>
                      </div>
                      <div className="bg-black/40 rounded-xl p-4 border border-white/5 flex items-center justify-between text-[10px] italic">
                         <p className="text-zinc-400 pr-4">{m.stats.reasoning}</p>
                      ...
                      <div className="flex gap-2">
                         ...
                         <button onClick={() => handleSendMessage(`Make this more concise`)} className="flex-1 py-3 bg-zinc-800 hover:bg-emerald-600 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-2 transition-all"><DollarSign size={12}/> Cheaper</button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8 bg-gradient-to-t from-[#050506] to-transparent z-10">
          <div className="max-w-3xl mx-auto relative group">
            <textarea 
              ...
              placeholder="Paste a prompt and save money..." 
