...
import { Droplets, DollarSign, Zap, Shield } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#09090b] text-white font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      <header className="relative py-24 px-6 text-center max-w-6xl mx-auto flex flex-col items-center z-10">
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 mb-10 p-2 bg-zinc-900/40 border border-zinc-800/50 backdrop-blur-md rounded-2xl sm:rounded-full">
          <div className="px-4 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[9px] sm:text-[10px] font-black tracking-widest uppercase">Private Beta</div>
          ...
          <div className="flex items-center gap-2 px-3 border-l border-zinc-800/50">
            ...
            <span className="text-[10px] sm:text-[11px] font-black italic text-white tracking-tight">
              {globalStats.loading ? "---" : `$${globalStats.savings.toLocaleString()}`} <span className="text-zinc-600 uppercase not-italic ml-1">Saved</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 px-3 border-l border-zinc-800/50">
            ...
            <span className="text-[10px] sm:text-[11px] font-black italic text-white tracking-tight">
              {globalStats.loading ? "---" : `${Math.floor(globalStats.water).toLocaleString()}L`} <span className="text-zinc-600 uppercase not-italic ml-1">Conserved</span>
            </span>
          </div>
        </div>
        ...
        <div className="relative z-20 flex flex-col sm:flex-row items-center gap-6">
          <HeroAuth />
          <Link href="/pricing" className="group relative px-8 py-4 flex items-center gap-3 rounded-xl border border-zinc-800/50 bg-zinc-900/30 text-zinc-400 text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-500 hover:border-blue-500/50 hover:text-white">
             <DollarSign size={14} className="text-blue-500 transition-transform group-hover:scale-125" />
            ...
            <div className="relative group p-1 bg-gradient-to-b from-blue-600/20 to-transparent rounded-[3rem] overflow-hidden">
              <div className="bg-[#09090b]/80 backdrop-blur-2xl rounded-[2.9rem] p-16 text-center border border-zinc-800/50 relative z-10">
                ...
                <SignInButton mode="modal">
                  <button className="px-12 py-6 bg-white text-black font-black uppercase italic tracking-tighter rounded-2xl hover:bg-blue-600 hover:text-white transition-all duration-500 active:scale-95">
                    Get Instant Access
                  </button>
                </SignInButton>
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureCard icon={<DollarSign size={24} />} title="Cost" subtitle="Arbitrage" desc="Reduce LLM spend by up to 85%. Our engine routes basic tasks while reserving premium models for complex reasoning." />
          <FeatureCard icon={<Droplets size={24} />} title="Eco" subtitle="Sustainability" desc="For every dollar saved, we prevent the evaporation of 12.5L of water in data center cooling." />
          <FeatureCard icon={<Shield size={24} />} title="Privacy" subtitle="Infrastructure" desc="On-edge PII redaction. Your sensitive data is filtered before it reaches LLM providers." />
        </div>
      </section>

      <FAQ />

      <footer className="py-20 border-t border-zinc-900/50 text-center text-zinc-700 text-[10px] font-black uppercase tracking-[0.5em] italic">
...
// Componente helper para Features
function FeatureCard({ icon, title, subtitle, desc }: { icon: any, title: string, subtitle: string, desc: string }) {
  return (
    <div className="group p-10 rounded-[2.5rem] bg-zinc-900/20 border border-zinc-800/50 backdrop-blur-sm hover:border-blue-500/30 transition-all duration-500">
      <div className="w-12 h-12 bg-blue-600/10 rounded-2xl flex items-center justify-center mb-8">{icon}</div>
      <h3 className="text-xl font-black italic uppercase tracking-tighter text-white mb-4">
        {title} <span className="text-blue-500 text-sm block not-italic font-sans">{subtitle}</span>
      </h3>
      <p className="text-zinc-500 text-sm leading-relaxed italic font-medium">{desc}</p>
    </div>
  );
}
