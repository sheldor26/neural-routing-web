    ...
    <div className="min-h-screen bg-[#09090b] text-white font-sans selection:bg-blue-500/30">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl">
          ...
          <div className="space-y-4">
            <textarea 
              ...
              rows={3}
              placeholder="e.g., 'Summarize this text' or 'Write complex code for a fintech app'..."
              value={prompt}
          ...
          {result && (
            <div className="mt-10 p-8 bg-black border border-blue-500/20 rounded-[2rem] animate-in fade-in zoom-in duration-500">
              <div className="flex flex-wrap justify-between items-center gap-4 mb-6 pb-6 border-b border-zinc-800">
                ...
                <div className="text-zinc-500 font-mono text-sm">Latency: {result.routing.latency_ms}ms</div>
              </div>
              
              <div className="space-y-4">
                <p className="text-sm font-medium text-zinc-500">Intelligence Source: <span className="text-white">{result.routing.model_used}</span></p>
      ...
      <section id="features" className="max-w-7xl mx-auto px-6 py-32 grid md:grid-cols-3 gap-10">
        <FeatureCard 
          ...
          desc="P95 latency under 200ms. Our global edge network routes your requests to the fastest available inference nodes."
        />
        <FeatureCard 
          icon={<Shield className="text-blue-500" size={32} />}
          title="Enterprise Privacy"
          desc="Automatic PII filtering. We redact sensitive data locally before it ever touches external LLM providers."
        />
        <FeatureCard 
          icon={<BarChart3 className="text-blue-500" size={32} />}
          title="Multi-Tenant Analytics"
          desc="Real-time ROI insights. Track savings and performance per client, department, or individual API key."
        />
      </section>
      ...
      <section id="pricing" className="max-w-7xl mx-auto px-6 py-32">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-4">Scalable Pricing for Every Scale.</h2>
          <p className="text-zinc-500 text-lg">Save more as you grow. No hidden fees, just pure optimization.</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <PriceCard 
            tier="Starter"
            price="49"
            desc="Perfect for startups experimenting with AI routing."
            features={["Up to 5M Routed Tokens", "2 Client IDs", "Standard Support", "Basic Analytics"]}
          />
          <PriceCard 
            tier="Pro"
            price="199"
          ...
          <PriceCard 
            tier="Enterprise"
            price="Custom"
            desc="Full-scale optimization for large-scale operations."
            features={["Unlimited Tokens", "Unlimited Client IDs", "Dedicated Account Manager", "SLA Guarantee", "Custom Routing Logic"]}
          />
        </div>
      </section>
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <div className="p-10 rounded-[2.5rem] border border-zinc-800 bg-zinc-900/30 hover:border-blue-500/50 transition-all group hover:bg-zinc-900/50">
...
function PriceCard({ tier, price, desc, features, highlight = false }: { tier: string, price: string, desc: string, features: string[], highlight?: boolean }) {
  return (
    <div className={`p-10 rounded-[2.5rem] border ${highlight ? 'border-blue-600 bg-blue-600/5' : 'border-zinc-800 bg-zinc-900/30'} relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300`}>
      {highlight && (
        <div className="absolute top-0 right-0 bg-blue-600 text-white text-[10px] font-black uppercase px-4 py-1 rounded-bl-xl tracking-widest">
          Most Popular
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        {price !== "Custom" && <span className="text-zinc-500 text-sm ml-2">/mo</span>}
      </div>
      <p className="text-zinc-500 text-sm mb-8 leading-relaxed">{desc}</p>
